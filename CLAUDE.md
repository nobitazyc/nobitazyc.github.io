...
- Easter eggs: poking 30 headline letters (8 on touch) fires the confetti party (`egg`); the Konami code, or three quick taps on the character on a phone, toggles arcade mode (`html.arcade`: CRT overlay `.crt`, monospace caps, square shapes, WebAudio blips on page change, `LVL` counter). Esc leaves. The character's first line hints at the code. Pressing `.` (or long-pressing the brand on a phone) toggles the blueprint overlay (`.blue`, `blueDraw` in frame): grid, dashed outlines with config labels on live shapes, and a live panel of stage values — meant for the designer/engineer audience. The three eggs are tied together by achievements (`badges`, localStorage key `yz-badges`): Poet (letter party), Gamer (arcade), Engineer (blueprint). A bottom-right trophy pill appears after the first one; all three fire a final toast + party.
- Inner lists (`.gscroll`, `.xp .body`) must SCROLL THEMSELVES from wheel deltas and `preventDefault` every wheel event they swallow (`passive:false`). Chrome latches a wheel sequence to the scroller it started on (the page); a flick that lands the cursor on a list mid-sequence sends the momentum events to the list, and if the list only stops propagation without cancelling, Chrome applies that momentum to the latched page — this was the "Motion -> Work flick lands on Home" bug (2026-09-14). Never go back to `stopPropagation` alone.
- Syntax check after edits (every inline script block; the first `<script>` is the Clarity snippet, the main code is the last): `node -e "const fs=require('fs');const html=fs.readFileSync('index.html','utf8');for(const m of html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g))new Function(m[1])"`
