...
- Easter eggs: poking 30 headline letters (8 on touch) fires the confetti party (`egg`); the Konami code, or three quick taps on the character on a phone, toggles arcade mode (`html.arcade`: CRT overlay `.crt`, monospace caps, square shapes, WebAudio blips on page change, `LVL` counter). Esc leaves. The character's first line hints at the code.
